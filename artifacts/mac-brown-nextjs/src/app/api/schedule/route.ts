...
export async function GET() {
  ...
  try {
    ...
    const [gsRows, mcRows, wsRows, rowRows, showRows, priceRows, configRows, blockedRows] = json.valueRanges.map(
    ...
    // Blocked Dates tab: start (YYYY-MM-DD) | end (YYYY-MM-DD) | note (optional)
    // Mac adds rows here whenever he's out of town or fully booked on certain dates.
    const blockedDates = (blockedRows ?? [])
      .filter(row => cell(row, 0) && cell(row, 1))
      .map(row => ({
        start: cell(row, 0),
        end:   cell(row, 1),
        note:  cell(row, 2) || undefined,
      }));
