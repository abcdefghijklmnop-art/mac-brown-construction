...
const HAT_LINKS: Record<number, { url: string; total: number }> = {
  1: { url: "https://swipesimple.com/links/lnk_5bc8a50d",                     total: 32    },
  2: { url: "https://swipesimple.com/links/lnk_3a9bc4fa8fb74b799a0d5e8ae1f95349", total: 64    },
  3: { url: "https://swipesimple.com/links/lnk_af13409badd5637fe907d4e7f77b1a8f", total: 96    },
  4: { url: "https://swipesimple.com/links/lnk_8ef043c0b97c188cc777a859e604cef9", total: 128   },
};

const MAX_QTY = 4;

export default function HatQuantityCard() {
  const [qty, setQty] = useState(1);
  const { url, total } = HAT_LINKS[qty];
  ...
  return (
    <div className="group">
      <div className="relative aspect-video overflow-hidden mb-6 bg-muted">
        ...
        <img
          src={img("/images/mbff-hat.webp")}
          alt="Mac Brown Fly Fish Trucker Hat"
          className="w-full h-full object-contain group-hover:scale-105 transition-transform duration-700"
          loading="lazy"
        />
      </div>
      <p className="text-xs text-muted-foreground uppercase tracking-wider mb-1">Mac Brown Fly Fish</p>
      <div className="flex items-baseline justify-between mb-1">
        <h3 className="font-serif text-xl font-bold text-foreground leading-snug">
          Mac Brown Fly Fish Trucker Hat
        </h3>
        <span className="font-serif text-2xl font-bold text-foreground ml-4 shrink-0">$32</span>
      </div>
      <p className="text-xs text-muted-foreground mb-4">Shipping included</p>
      <p className="text-muted-foreground text-sm leading-relaxed mb-6">
        Waxed cotton trucker hat with embroidered fly and logo. Est. 1987, Bryson City, NC. One size fits most.
      </p>
      ...
      <div className="flex flex-col gap-4">
        ...
        <a
          ...
          className="inline-flex items-center justify-center gap-2 bg-primary text-primary-foreground text-xs font-semibold uppercase tracking-[0.15em] px-7 py-3 hover:bg-primary/90 transition-colors w-fit"
        >
          {qty === 1 ? `Buy Now — $${total}` : `Buy ${qty} Hats — $${total}`}
        </a>
