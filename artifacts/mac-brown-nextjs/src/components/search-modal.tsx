...
import { MBFF_SEARCH_INDEX, type SearchPage } from "@/lib/search-index";
...
export default function SearchModal({ open, onClose }: Props) {
  ...
  const results = useMemo<SearchPage[]>(() => {
    if (!query.trim()) return MBFF_SEARCH_INDEX.slice(0, 27);
    const q = query.toLowerCase();
    return MBFF_SEARCH_INDEX.filter(
      (p) =>
      ...
      <div className="relative w-full max-w-xl bg-background border border-border shadow-2xl flex flex-col overflow-hidden">
        <div className="flex items-center gap-3 px-4 py-3 border-b border-border">
          ...
          <input
            ...
            onKeyDown={handleKey}
            placeholder="Search pages, programs, schools…"
