...
export async function POST(req: NextRequest) {
  const { firstName, email, schoolName } = await req.json() as Record<string, unknown>;
  ...
  try {
    ...
    if (!emailRes.ok) {
      ...
      console.error("ffgs-review-request email failed:", err);
      return NextResponse.json({ error: "Email failed." }, { status: 500 });
    }

    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error("ffgs-review-request unexpected error:", err);
