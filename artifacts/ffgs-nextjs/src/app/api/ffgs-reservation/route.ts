...
export async function POST(req: NextRequest) {
  ...
  const webhookUrl = process.env.FFGS_SHEET_WEBHOOK_URL;
  if (webhookUrl) {
    try {
      const row = [
        submittedAt,
      ...
      if (!sheetRes.ok) {
        console.error("Sheet webhook failed:", await sheetRes.text());
      }
    } catch (err) {
      console.error("Sheet webhook error — reservation still proceeding:", err);
    }
  }
  ...
  if (apiKey && !isCard && email) {
    try {
          ...
          html: `
            <div style="font-family: Georgia, serif; max-width: 600px; margin: 0 auto; color: #1a1a1a;">
              <div style="border-bottom: 2px solid #1a1a1a; padding-bottom: 16px; margin-bottom: 24px;">
                ...
                <h1 style="font-size: 28px; font-weight: bold; margin: 8px 0 0;">Fly Fishing Guide School</h1>
              </div>
              ...
              <div style="background: #f5f5f0; border-left: 3px solid #1a1a1a; padding: 16px 20px; margin: 24px 0;">
                <p style="font-size: 11px; letter-spacing: 0.15em; text-transform: uppercase; color: #666; margin: 0 0 8px;">Reservation Summary</p>
              ...
              <div style="background: #fff8e1; border: 1px solid #f0c040; padding: 14px 18px; margin: 20px 0; font-size: 14px;">
                <strong>Venmo reminder:</strong> Send to <strong>@James-Brown3656</strong> and include your name and school month in the note.
              </div>` : ""}
              ${(depositType as string)?.toLowerCase().includes("zelle") ? `
              <div style="background: #fff8e1; border: 1px solid #f0c040; padding: 14px 18px; margin: 20px 0; font-size: 14px;">
                <strong>Zelle reminder:</strong> Send to <strong>(828) 736-1469</strong> and include your name and school month in the memo.
              </div>` : ""}
              ${(depositType as string)?.toLowerCase().includes("check") ? `
              <div style="background: #fff8e1; border: 1px solid #f0c040; padding: 14px 18px; margin: 20px 0; font-size: 14px;">
                <strong>Check reminder:</strong> Make payable to <strong>Fly Fishing Guide School</strong> and mail to:<br/>
                <span style="font-family: monospace; display: block; margin-top: 8px;">779 West Deep Creek Rd<br/>Bryson City, NC 28713</span>
              </div>` : ""}
              <p style="font-size: 15px; line-height: 1.6;">
                Questions? Call us at <a href="tel:+18287361469" style="color: #1a1a1a;">(828) 736-1469</a> or reply to this email.
              </p>
      ...
      console.warn("Student confirmation email failed — reservation still saved:", emailErr);
    }
  }
  ...
  if (apiKey) {
    try {
        ...
        body: JSON.stringify({
          ...
          html: `
            <div style="font-family: Arial, sans-serif; max-width: 560px; margin: 0 auto; color: #222;">
              <h2 style="border-bottom: 2px solid #2c4a7c; padding-bottom: 10px; color: #2c4a7c;">New Reservation Received</h2>
              <table style="width:100%; font-size:14px; border-collapse:collapse;">
                ...
                <tr><td style="padding:6px 0; color:#666;">Street Address</td><td style="padding:6px 0;">${streetAddress ?? "—"}</td></tr>
                <tr><td style="padding:6px 0; color:#666;">City/State/Zip</td><td style="padding:6px 0;">${cityStateZip ?? "—"}</td></tr>
                <tr><td style="padding:6px 0; color:#666;">Emergency Contact</td><td style="padding:6px 0;">${emergencyContact ?? "—"} · ${emergencyPhone ?? "—"}</td></tr>
                <tr><td style="padding:6px 0; color:#666;">Medical</td><td style="padding:6px 0;">${medicalConditions ?? "None"}</td></tr>
      ...
      console.warn("School notification email failed — reservation still saved:", notifyErr);
    }
  }
