...
router.post("/mbff-review-request", async (req, res) => {
  const { firstName, email, programName } = req.body as Record<string, unknown>;
  ...
  try {
    ...
    if (!emailRes.ok) {
      ...
      req.log.error({ err, email }, "mbff-review-request email failed");
      res.status(500).json({ error: "Email failed." });
      return;
    }

    req.log.info({ email, name, program }, "MBFF review request sent");
    res.json({ ok: true });
  } catch (err) {
    req.log.error(err, "mbff-review-request unexpected error");
